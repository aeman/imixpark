 ...
 * 日    期：12-2-29
 */
package cn.ebatech.imixpark.rop;

import cn.ebatech.imixpark.demo.model.User;
import cn.ebatech.imixpark.demo.service.UserService;
import cn.ebatech.imixpark.rop.request.CreateUserRequest;
import cn.ebatech.imixpark.rop.request.LogonRequest;
import cn.ebatech.imixpark.rop.request.UploadUserPhotoRequest;
import cn.ebatech.imixpark.rop.response.*;
...
public class SampleUserService extends AbstractUserService{
...
//    }
    ...
    public Object getSession(LogonRequest request) {
        ...
        request.getRopRequestContext().addSession("mockSessionId1222", session);

        //返回响应
        LogonResponse logonResponse = new LogonResponse();
        logonResponse.setSessionId("mockSessionId1222");
    ...
    public Object logon(LogonRequest request) {
        ...
        SimpleSession session = new SimpleSession();
        session.setAttribute("userName",request.getUserName());
        request.getRopRequestContext().addSession("mockSessionId1111", session);

//        getUser("http://localhost:8888/services/users/1.json");

        User user = new User(1L, "托马斯da火车");
        System.out.println("SUCCESS: registered user with id " + userService.registerUser(user));

        RpcContext.getContext().setAttachment("clientName", "rop");
        RpcContext.getContext().setAttachment("clientImpl", "dubbox");
        System.out.println("SUCCESS: got user " + userService.getUser(1L));

        //返回响应
        LogonResponse logonResponse = new LogonResponse();
        logonResponse.setSessionId("mockSessionId1111");
        return logonResponse;
    }
